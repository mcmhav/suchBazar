...
def handleStraight():
    ...
    e = open('userTouser' + '.dot','w')
    e.write("digraph userTouser {\n")

    usersNew = writeUsers(users,e,total)

    writeConnncetions(usersNew,users,total,e)

    e.write("}")

    e.close()

def writeConnncetions(usersNew,users,total,e):
    handledUsers = []
    count = 0
    for user in users:
